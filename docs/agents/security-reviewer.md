...
- Both flags are guarded `!== false`, not by a truthy check, and both default to the truthy direction. A truthy check fails **open** for any consumer whose shipped config predates the key. The integration tier cannot see this: with the shipped default `true`, a `=== true` guard leaves every integration test green. Only `test/unit/request-test.ts` can distinguish it — #50's AC3 for `strictRoutes`, #47's AC6 for `caseSensitiveRoutes`. Both cover *two* failure shapes, and the distinction matters: a guard that fails open only when the key is absent as an own property survives a present-and-`undefined` assertion. Both ACs therefore probe the key present-and-`undefined` **and** deleted as an own property, each with a `hasOwnProperty` precondition assertion. Neither `sinon.stub().value()` nor any other sinon call can express the absent case — `stub()` either materializes an own property or throws — so those assertions manipulate `config.restServer` directly and restore the prior own-property state in `afterEach`. Flag any change that weakens either guard's polarity or deletes those unit assertions
